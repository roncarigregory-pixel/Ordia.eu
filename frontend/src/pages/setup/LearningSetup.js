...
export default function LearningSetup() {
  ...
  const remove = async (it) => {
    try { await api.delete(`/learning/${it.id}`); load(); toast.success(t("Regola rimossa")); }
  ...
  return (
    <div className="animate-fade-up max-w-3xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-display text-3xl font-black tracking-tighter">{t("Apprendimento")}</h1>
          <p className="text-sm text-muted-foreground">{t("Ogni ordine validato insegna a Ordia. Queste regole migliorano i prossimi abbinamenti.")}</p>
        </div>
      </div>

      <div className="relative max-w-xs mb-4">
        <MagnifyingGlass size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
        <input data-testid="learning-search" value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("Cerca frase o prodotto…")} className="w-full rounded-md border border-input bg-white pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring" />
      </div>
      ...
      <div className="rounded-md border border-border bg-white overflow-hidden">
        {!items ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="mt-3 text-sm text-muted-foreground">{t("Ancora nessuna regola appresa. Valida qualche ordine e Ordia inizierà a imparare.")}</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left">
                <th className="px-5 py-3 text-xs font-medium uppercase tracking-[0.15em] text-muted-foreground">{t("Frase cliente")}</th>
                <th className="px-5 py-3 text-xs font-medium uppercase tracking-[0.15em] text-muted-foreground">{t("→ Prodotto")}</th>
                <th className="px-5 py-3 text-xs font-medium uppercase tracking-[0.15em] text-muted-foreground text-center">{t("Volte")}</th>
