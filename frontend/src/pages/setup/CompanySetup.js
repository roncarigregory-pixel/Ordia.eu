  ...
  return (
    <div className="animate-fade-up max-w-2xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="font-display text-3xl font-black tracking-tighter">Dati azienda</h1>
          <p className="text-sm text-muted-foreground">Compaiono su export ed email in uscita.</p>
        </div>
      </div>

      <div className="rounded-md border border-border bg-white p-6 space-y-4">
        <Field label="Ragione sociale" testid="company-name">
          <input value={form.name} onChange={set("name")} className={inputCls} />
        </Field>
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Partita IVA" testid="company-vat">
            <input value={form.vat} onChange={set("vat")} placeholder="IT01234567890" className={inputCls} />
          </Field>
          <Field label="Telefono" testid="company-phone">
            <input value={form.phone} onChange={set("phone")} className={inputCls} />
          </Field>
        </div>
        <Field label="Indirizzo" testid="company-address">
          <input value={form.address} onChange={set("address")} className={inputCls} />
        </Field>
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Paese" testid="company-country">
            <input value={form.country} onChange={set("country")} placeholder="Italia" className={inputCls} />
          </Field>
          <Field label="Valuta" testid="company-currency">
